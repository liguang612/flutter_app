...
abstract class Indicator extends StatelessWidget
  ...
  /// Create a dot with number indicator.
  ///
  /// See also:
  ///
  /// * [numberDotIndicator], exactly the same
  factory Indicator.numberDot({
    Key? key,
    double? size,
    Color? color,
    double? position,
...
/// A widget that displays dot with number.
class numberDotIndicator extends Indicator {
  /// Creates a number dot indicator.
  ///
  /// The [size] must be null or non-negative.
  const numberDotIndicator({
    Key? key,
    double? size,
    Color? color,
    double? position,
  ...
  Widget build(BuildContext context) {
    ...
    return Center(
      child: Container(
        width: effectiveSize ?? ((child == null) ? 20.0 : null),
        height: effectiveSize ?? ((child == null) ? 20.0 : null),
        child: Text(
          '$number',
